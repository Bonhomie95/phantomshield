...
Building locally instead (`npx expo run:ios` / `run:android`)? Export
`SENTRY_DISABLE_AUTO_UPLOAD=true` first. The bundle step tries to upload source
maps, has no Sentry token outside EAS, and fails the whole build without it.
`ios/` and `android/` are generated by prebuild, so anything set inside them is
